...
hMain = figure('Position',[30 30 .7*scrsz(3) .85*scrsz(4)],'resize','off','toolbar','figure');
...
hD3H = uicontrol('parent',hMain,'units','normalized','position',[.05 .40 .3 .06],...
    'style','text','fontsize',15,'fontweight','bold','string','Drifter',...
...
hD4H = uicontrol('parent',hMain,'units','normalized','position',[.05 .12 .3 .06],...
    'style','text','fontsize',12,'fontweight','bold','string','Ship Position',...
...
%     load testWebData
    ...
    lastDrift1 = max(find(deviceNum==5));
    lastWire= max(find(deviceNum==7));
...
%     [rangeShipToDrift, AF2, AR2]=dist([datLat(lastShip,1) datLat(lastDrift1,1)],[datLon(lastShip,1) datLon(lastDrift1,1)]);
    [rangeShipToSed, AF1, AR1]=dist([latDD datLat(lastSed,1)],[lonDD datLon(lastSed,1)]);
    [rangeShipToDrift, AF2, AR2]=dist([latDD datLat(lastDrift1,1)],[lonDD datLon(lastDrift1,1)]);
    [rangeShipToWire, AF3, AR3]=dist([latDD datLat(lastWire,1)],[lonDD datLon(lastWire,1)]);
    ...
    if  ~isempty(rangeShipToSed)
        ...
        set(hD1L,'string',{pos1;['Range from Ship: ' num2str(rangeShipToSed/1000) 'km'];...
            ['Bearing from Ship:  ' num2str(round(AF1))]});
    end
    
    if  ~isempty(rangeShipToWire)
        if AF3<0
            AF3=AF3+360;
        end
        set(hD2L,'string',{pos4;['Range from Ship: ' num2str(rangeShipToWire/1000) 'km'];...
            ['Bearing from Ship:  ' num2str(round(AF3))]});
    end
    ...
    if  ~isempty(rangeShipToDrift)
        ...
        set(hD3L,'string',{pos3;['Range from Ship: ' num2str(rangeShipToDrift/1000) 'km'];...
